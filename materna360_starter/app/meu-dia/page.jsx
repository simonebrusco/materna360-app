...
import { getMessage, nextMessage } from "../../lib/messages";
...
export default function MeuDiaPage() {
  ...
  // Carrega planner do storage
  useEffect(() => {
    setMsg(getMessage());
    setPlanner(get(keys.planner, { casa: [], filhos: [], eu: [] }));
  }, []);

  // Progresso por aba e total
  const pct = useMemo(() => {
  ...
  return (
    <main>
      ...
      {/* Planner da Família (Widget na Home) */}
      ...
      {openNote && <QuickNote onClose={() => {
        setOpenNote(false);
        // recarrega resumo após adicionar nota
        setPlanner(get(keys.planner, { casa: [], filhos: [], eu: [] }));
      }} />}
    </main>
  );
