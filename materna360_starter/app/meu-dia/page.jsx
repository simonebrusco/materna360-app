// app/meu-dia/page.jsx  (RAIZ do projeto)
"use client";
import Link from "next/link";
import { useEffect, useMemo, useState } from "react";

// ✅ importa do materna360_starter (um nível só)
import QuickNote from "../materna360_starter/components/QuickNote";
import { getMessage, nextMessage } from "../materna360_starter/lib/messages";
import { get, keys } from "../materna360_starter/lib/storage";
...
export default function MeuDiaPage() {
  ...
  const pct = useMemo(() => {
    ...
    const all = calcPct([...(planner.casa||[]), ...(planner.filhos||[]), ...(planner.eu||[])]);
    ...
    <main>
      ...
      <section className="rounded-2xl p-6 bg-gradient-to-b from-rose-100 to-rose-50 mb-5">
        <div className="flex items-center justify-between">
          ...
          <Link href="/eu360" className="text-xs bg-white/70 backdrop-blur px-3 py-1 rounded-full">Eu360</Link>
        </div>
        ...
        <div className="mt-2 text-[15px] text-slate-600 max-w-[48ch] flex items-start gap-2">
          ...
          <div className="flex-1">
            <div>{msg}</div>
            <button onClick={novaMensagem} className="text-brand text-sm hover:underline mt-1">Nova mensagem</button>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {CARDS.map((c) => (
          <Link key={c.href} href={c.href} className="card flex gap-3 items-center hover:shadow-md transition-shadow">
      ...
      {/* Planner na Home (widget) */}
      <section className="card mb-6">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-lg font-semibold">Planner da Família</h4>
          <Link href="/meu-dia/planner" className="btn bg-white border border-slate-200">Abrir Planner</Link>
        </div>
      ...
      <section className="card">
        ...
        <p className="text-xs text-slate-400 mt-3">Dica: encontre um momento para se cuidar hoje 💗</p>
      </section>
